...
# So the co2 is made up of:
  # the ICE fraction; where it is the VKTs travelled, multiplied by the rate of diesel consumption
...
#' The current dataset contains the emissions rates for each scenario we are interested in - some
#' are in per km (non-exhaust) and some are in per L (exhaust). Secondary pollutants which
#' are going to be calculated as a conversion rate from primary exhaust pollutants are not
#' yet included
#' 
...
health_costs <- policy_outcomes %>%
  filter(fleet_year %in% c(2022:2040)) %>%
  ...
  ungroup() %>%
  mutate(health_cost_total = (1 / (1 + 0.07)^(fleet_year - 2022)) * health_cost_total) # ---------apply 7% discount to future health costs


# ----------------

IB_play <- health_costs %>%
  filter(scenario %in% c("Euro 6 (2024)",
                         "baseline"))%>% 
  filter(fleet_year %in% c(2020:2040)) %>%
  group_by(scenario, vkt_scenario, fleet_year) %>% 
  summarise(total_cost = sum(health_cost_total)) %>%
  ungroup() %>%
  group_by(vkt_scenario) %>%  
  mutate(cum_health = cumsum(total_cost)) %>%
  select(-total_cost) %>%
  pivot_wider(values_from = cum_health,
...
# Have a look at calculating savings between the two scenarios in aggregate?
